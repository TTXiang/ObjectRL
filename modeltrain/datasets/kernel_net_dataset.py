...
def load_image(self, video_index):
    ...
    return imgs_re, self.image_size, (new_size0, new_size1), file_num  # img, hw_original, hw_resized
...
class kernel_Dataset(Dataset):
    def __init__(self, files_row_root, rank=-1,
        ...
        for video_file in os.listdir(self.files_row_root):
            ...
            label_single = []
            imgs_single = []
            visible_files = [f for f in os.listdir(os.path.join(video_files, 'color')) if f.endswith('.jpg')]
            ...
            if frame_num >= 2:
                ...
                for i, file in enumerate(pbar):
                    ...
                    if l.shape[0]:
                        ...
                        labels[i] = np.zeros((0, 5), dtype=np.float32)
                        ne += 1  # file empty
    ...
    def __getitem__(self, video_index):
        # load image
        imgs_re, self.image_size, (w, h), file_num = load_image(self, video_index)
        self.ratio_p = 1 / self.ratio

        # load labels
        labels_sum = [None] * file_num
        ...
        for x_label in x:
            ...
            labels_sum[fps_index] = labels_out
            fps_index += 1

        return imgs_re, labels_sum, self.image_size, (h, w)
...
class kernel_infer_Dataset(Dataset):
    ...
    def __getitem__(self, video_index):
        # load image
        imgs_re, self.image_size, (w, h) = load_image(self, video_index)
        self.ratio_p = 1 / self.ratio
